...
// NOTE: for this I confused owner = deployer
// I created the contract with signer which then became owner, so the script did not work!
// a. Easier. From the template:
// those basic methods are open source and get them from ERC-20 standard
...
require('dotenv').config({ path: "../.env" }); //important when executing from inside hardhat project!
const ethers = require("ethers");
console.log(ethers.version);

// Update info to match your contract.
const cAddress = "0xBe3E899A80C6606852961834f21843247A9eFEC0";
const cName = "MyERC20";
...
let signer = new ethers.Wallet(process.env.METAMASK_PRIVATE_KEY, notUniMaProvider);
...
const transfer = async () => {
    ...
    let balanceReceiver = await contract.balanceOf(deployer.address);
    ...
    console.log(deployer.address)
    let tx = await contract.transfer(deployer.address, amountToTransfer);
    await waitForTx(tx, true);
    
    // Check balances.
    let balance2 = await contract.balanceOf(signer.address);
    console.log("Updated sender balance: ", Number(balance2));
    let balanceReceiver2 = await contract.balanceOf(deployer.address);
...
const transferFrom = async () => {
    // Get contracts.
    const contract = await getContract(signer);
    const contractReceiver = await getContract(deployer);
    ...
    tx = await contract.transferFrom(signer.address, deployer.address, amountToTransfer);
    await waitForTx(tx);
    ...
    // Check allowance.
    let allowance = await contract.allowance(signer.address, deployer.address);
